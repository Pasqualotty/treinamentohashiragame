...
## Player canônico unificado (movimento + combate MVP).
## Cena: scenes/characters/player/player.tscn
##
## State machine: idle / run / jump / dash / attack_basic / skill_1 / skill_2 /
## ultimate / hurt / dead.
##
## InputMap (só actions — sem is_key_pressed):
##   move_left/right · jump · advance(dash) · attack_basic · skill_1 · skill_2 · ultimate
##
## Teclas playtest (project.godot):
##   A/D ou setas · Espaço pulo · Shift/F dash · Z/J atk · X/K skill1 · C/L skill2 · V/I ult
##
## Regras GDD:
##   - Pulo 1× + coyote; dash c/ cooldown **sem** i-frames
...
func try_dash() -> bool:
	## Dash curto na direção que olha. Cooldown; 1 dash aéreo. Sem i-frames.
	if _state == State.DASH or _state == State.DEAD or _state == State.HURT:
		return false
	if _is_attack_locked():
		return false
...
func _process_locomotion(delta: float) -> void:
	...
	# Combate — prioridade: ultimate > skills > basic (só no chão p/ MVP skills/atk)
...
func _process_action(delta: float) -> void:
	...
	# Skill 2 investida: avança durante active.
...
func _on_hitbox_hit(_hurtbox: Hurtbox, _hit_data: HitData) -> void:
	# Respiração por hit que acerta (GDD).
	if stats:
...
func _update_hurtbox_invuln() -> void:
	if hurtbox == null or _state == State.DEAD:
		return
	# i-frames de hurt/ultimate; dash NÃO dá i-frames.
